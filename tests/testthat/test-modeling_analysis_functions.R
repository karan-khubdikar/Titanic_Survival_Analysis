...
test_that("`fit_logistic_model` should return a `glm` class", {
  expect_s3_class(fit_logistic_model(data, "y", c("x1", "x2")), "glm")
})

test_that("`fit_logistic_model` should return a binomial family object with `logit` link function", {
  expect_equal(fit_logistic_model(data, "y", c("x1", "x2"))$family$family, "binomial")
  expect_equal(fit_logistic_model(data, "y", c("x1", "x2"))$family$link, "logit")
})

test_that("`fit_logistic_model` should return correct estimated values", {
